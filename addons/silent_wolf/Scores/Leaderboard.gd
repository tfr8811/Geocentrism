...
func render_board(scores: Array, local_scores: Array) -> void:
	...
	if all_scores.is_empty():
		...
		# add the 10th item as the gatekeeper score
		if (scores.size() >= max_scores):
			GlobalWorldState.GatekeeperScore = scores[max_scores-1].score
	else:
		for score in all_scores:
			add_item(score.player_name, str(int(score.score)))
		# add the 10th item as the gatekeeper score
		if (all_scores.size() >= max_scores):
			GlobalWorldState.GatekeeperScore = all_scores[max_scores-1].score
